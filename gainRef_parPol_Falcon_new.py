#!/usr/bin/env python
#for coverting star file to do particle polishing using john rubinstein's program 2015
###
...
def process_starfile(args1):
	...
	for kk in klines:
		...
		if len(itemss) > 3 : 
			if itemss[micrographN] not in fullmicrographs:
				...
				f = re.split(r'/|\.',itemss[micrographN])[1] 
	...
	for i in fullmicrographs:
		...
		for index,pp in enumerate(dd[microitems[1]]):
			itemsa = pp.split()
			xpos_relion = int (float (itemsa[coordninateXN])) + 10
			ypos_relion = int (float (itemsa[coordninateYN])) + 10
...
def parse_command_line():
	...
	parser.add_option("--nx",dest="nx", metavar="<n>", type='int', help="number of pixels of the micrographs of x axis, default to 4096", default=4096)
	parser.add_option("--ny",dest="ny", metavar="<n>", type='int', help="number of pixels of the micrographs of y axis, default to 4096", default=4096)
